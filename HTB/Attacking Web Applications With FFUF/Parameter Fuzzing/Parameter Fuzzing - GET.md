...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ ffuf -w /opt/useful/SecLists/Discovery/Web-Content/burp-parameter-names.txt:FUZZ -u http://admin.academy.htb:PORT/admin/admin.php?FUZZ=key -fs xxx
...
`user`
* run command `ffuf -w /usr/share/seclists/Discovery/Web-Content/burp-parameter-names.txt:FUZZ -u http://admin.academy.htb:31958/admin/admin.php?FUZZ=key`, make sure you change the txt file by using the one on the `Web-Content` folder called `burp-parameter-names.txt`
* again, we get a lot of responses which we dont need, we can filter them by adding a filter based on their size, which is 798
* run command `ffuf -w /usr/share/seclists/Discovery/Web-Content/burp-parameter-names.txt:FUZZ -u http://admin.academy.htb:31958/admin/admin.php?FUZZ=key -fs 798`, the only result will be `user`