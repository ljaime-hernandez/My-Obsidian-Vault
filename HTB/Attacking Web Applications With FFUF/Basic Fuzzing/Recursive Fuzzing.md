...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ ffuf -w /opt/useful/SecLists/Discovery/Web-Content/directory-list-2.3-small.txt:FUZZ -u http://SERVER_IP:PORT/FUZZ -recursion -recursion-depth 1 -e .php -v
...
HTB{fuzz1n6_7h3_w3b!}
* run command `ffuf -w /usr/share/seclists/Discovery/Web-Content/common.txt:FUZZ -u http://159.65.49.34:31035/FUZZ -recursion -recursion-depth 1 -e .php -v`
* one of the results will be `http://159.65.49.34:31035/forum/flag.php` (as the rest of the results are either unnecessary php files or folders + files from previous exercises)
* copy the link on firefox and retrieve the flag