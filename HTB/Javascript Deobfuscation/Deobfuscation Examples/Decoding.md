...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ curl http:/SERVER_IP:PORT/serial.php -X POST -d "param1=sample"
...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ echo https://www.hackthebox.eu/ | base64
...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ echo aHR0cHM6Ly93d3cuaGFja3RoZWJveC5ldS8K | base64 -d
...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ echo https://www.hackthebox.eu/ | xxd -p
...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ echo 68747470733a2f2f7777772e6861636b746865626f782e65752f0a | xxd -p -r
...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ echo https://www.hackthebox.eu/ | tr 'A-Za-z' 'N-ZA-Mn-za-m'
...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ echo uggcf://jjj.unpxgurobk.rh/ | tr 'A-Za-z' 'N-ZA-Mn-za-m'
...
* use `N2gxNV8xNV9hX3MzY3IzN19tMzU1NGcz` on [boxentriq](https://www.boxentriq.com/code-breaking/cipher-identifier), it will identify the encode as base64
* on command line type `echo N2gxNV8xNV9hX3MzY3IzN19tMzU1NGcz | base64 -d` and retrieve the decoded message which is `7h15_15_a_s3cr37_m3554g3`
* add the decoded message on the parameter as serial using this command `curl http://<IP>:<PORT>/serial.php -X POST -d "serial=7h15_15_a_s3cr37_m3554g3"`