...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ cat /etc/exports 
...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ sudo nmap 10.129.14.128 -p111,2049 -sV -sC
...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ sudo nmap --script nfs* 10.129.14.128 -sV -p111,2049
...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ showmount -e 10.129.14.128
...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ mkdir target-NFS
Luis Miguel Jaime Hernandez@htb[/htb]$ sudo mount -t nfs 10.129.14.128:/ ./target-NFS/ -o nolock
Luis Miguel Jaime Hernandez@htb[/htb]$ cd target-NFS
...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ ls -l mnt/nfs/
...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ ls -n mnt/nfs/
...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ cd ..
Luis Miguel Jaime Hernandez@htb[/htb]$ sudo umount ./target-NFS
```
...
* run command `sudo nmap -sC -sV -p111,2049 --script nfs* <ip given>`, the result will show 2 available shares, one named `nfs` and `nfsshare`, both with all permissions for you to mount a folder on it
* run command `sudo mkdir <any folder name>` which will be mounted on the target
* run command `sudo mount -t nfs <ip given>:/ ./<your folder name>/ -o nolock`, the `-t` stands for the target, the `nfs` will indicate an `nfs directory` which is what we will mount on the target and the `-o` means the option we want to choose for it, which will be `nolock`
	* if the command gives you this error  `bad option; for several filesystems (e.g. nfs, cifs) you might need a /sbin/mount.<type> helper program.` youll need to install a package called `nfs-common`. If needed, run the command `sudo apt-get install nfs-common`
* open the folder you just mounted from your local computer
* open each folder until you get to the `nfs` one, open the `flag.txt` to get the answer
