...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ curl -X POST -d 'username=admin&password=admin' http://<SERVER_IP>:<PORT>/
...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ curl -X POST -d 'username=admin&password=admin' http://<SERVER_IP>:<PORT>/ -i
...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ curl -b 'PHPSESSID=c1nsa6op7vtk7kdis7bcnbadf1' http://<SERVER_IP>:<PORT>/
...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ curl -X POST -d '{"search":"london"}' -b 'PHPSESSID=c1nsa6op7vtk7kdis7bcnbadf1' -H 'Content-Type: application/json' http://<SERVER_IP>:<PORT>/search.php
["London (UK)"]
```
...
`HTB{p0$t_r3p34t3r}`
curl -d '{"search":"flag"}' 
      -H 'Cookie: PHPSESSID=kcnkikmvso771l346utc49iau5' 
      -H 'Content-Type: application/json' 
      -v http://178.128.37.153:30914/search.php

-d to infere a json POST, closed with single colons, otherwise it wont work
-H to include the Cookie so we can do the search as an authenticated user
-H again to set the post body type in json format
-v for the reply to be verbose so we can receive the flag
