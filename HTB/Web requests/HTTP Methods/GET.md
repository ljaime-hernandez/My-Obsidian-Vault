...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ curl -i http://<SERVER_IP>:<PORT>/
...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ curl -u admin:admin http://<SERVER_IP>:<PORT>/
...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ curl http://admin:admin@<SERVER_IP>:<PORT>/
...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ curl -v http://admin:admin@<SERVER_IP>:<PORT>/
...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ curl -H 'Authorization: Basic YWRtaW46YWRtaW4=' http://<SERVER_IP>:<PORT>/
...
![network_clear_requests](https://academy.hackthebox.com/storage/modules/35/network_clear_requests.jpg)
...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ curl 'http://<SERVER_IP>:<PORT>/search.php?search=le' -H 'Authorization: Basic YWRtaW46YWRtaW4='
...
`HTB{curl_g3773r}`
curl -i -v http://admin:admin@138.68.183.154:30980/search.php?search=flag

response:

*   Trying 138.68.183.154:30980...
* Connected to 138.68.183.154 (138.68.183.154) port 30980 (#0)
* Server auth using Basic with user 'admin'
> GET /search.php?search=flag HTTP/1.1
> Host: 138.68.183.154:30980
> Authorization: Basic YWRtaW46YWRtaW4=
...
< HTTP/1.1 200 OK
HTTP/1.1 200 OK
< Date: Wed, 11 Jan 2023 18:26:38 GMT
Date: Wed, 11 Jan 2023 18:26:38 GMT
< Server: Apache/2.4.41 (Ubuntu)
Server: Apache/2.4.41 (Ubuntu)
< Cache-Control: no-cache, must-revalidate, max-age=0
...
* Connection #0 to host 138.68.183.154 left intact
