...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ sudo nmap 10.129.2.28 -p- -sV
...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ sudo nmap 10.129.2.28 -p- -sV --stats-every=5s
...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ sudo nmap 10.129.2.28 -p- -sV -v 
...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ sudo nmap 10.129.2.28 -p- -sV -Pn -n --disable-arp-ping --packet-trace
...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ sudo tcpdump -i eth0 host 10.10.14.2 and 10.129.2.28
...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$  nc -nv 10.129.2.28 25
...
* run command `nmap 10.129.95.26 -sV` to run a scan in most default ports retrieving its service banners, once the scan is done, notice theres an anomaly in port `31337`, which states the service as `Elite?` and take it from there
* run command `nc -nv 10.129.95.26 31337`, this will retrieve banners which nmap wont retrieve by default, once the scan is done you will be able to retrieve the HTB flag