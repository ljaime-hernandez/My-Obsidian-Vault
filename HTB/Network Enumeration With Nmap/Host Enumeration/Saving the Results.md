...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ sudo nmap 10.129.2.28 -p- -oA target
...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ cat target.gnmap
...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ cat target.xml
...
```shell-session
Luis Miguel Jaime Hernandez@htb[/htb]$ xsltproc target.xml -o target.html
```
